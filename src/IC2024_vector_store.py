...
def main(config_path):
    ...
    create_vector_store(img_embs, "IC2024_image_faiss.index", config['folders']['vector_store_dir'])
    create_vector_store(txt_embs, "IC2024_text_faiss.index", config['folders']['vector_store_dir'])



if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pré-processa embeddings do ImageClef 2024 - Treino"
    )

    parser.add_argument(
        "--config",
        type=str,
        default="configs/IC2024_vector_store.yaml",
